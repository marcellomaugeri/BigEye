...
class TestImageBuilder:
    def test_build_uses_engine_api_with_exact_context_platform_tag_and_forwards_logs(self, tmp_path: Path) -> None:
        from backend.fuzzing.docker.image_builder import (
            IMAGE_BUILD_REQUEST_TIMEOUT_SECONDS,
            ImageBuilder,
        )
        ...
        assert IMAGE_BUILD_REQUEST_TIMEOUT_SECONDS == 21_600
        assert calls == [{"path": str(tmp_path), "dockerfile": "Dockerfile", "tag": "bigeye-llvm:test", "platform": "linux/amd64", "decode": True, "rm": True, "timeout": IMAGE_BUILD_REQUEST_TIMEOUT_SECONDS}]
