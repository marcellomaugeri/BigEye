...
class ProjectRepository:
    ...
    async def list_unfinished(self) -> list[Project]:
        rows = await self._pool.fetch(
            """SELECT id, repository_url, requested_revision, worker_count, commit_sha,
                      repository_token IS NOT NULL AS token_present, created_at, paused_at, error
               FROM projects WHERE paused_at IS NULL AND error IS NULL ORDER BY created_at, id"""
        )
        return [self._project(row) for row in rows]
    ...
    async def pause(self, project_id: int) -> None:
        await self._pool.execute("UPDATE projects SET paused_at = CURRENT_TIMESTAMP WHERE id = $1", project_id)

    async def resume(self, project_id: int) -> None:
        await self._pool.execute("UPDATE projects SET paused_at = NULL WHERE id = $1", project_id)
